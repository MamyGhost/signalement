...
"group by Statut.id order By Statut.id asc",nativeQuery=true)
   public List<Integer>findGroupByStatut(); 
    ...
    @Modifying
    @Query(value = "update signalement s set s.statut.etat=3 where s.description= :description" ,nativeQuery=true)
    public void statutUpdate(@Param("description") String description);
